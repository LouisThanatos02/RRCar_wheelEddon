...
PORT = 8000
connection = False
message_1st = 'Connect request'
client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
try:
   client.connect((HOST, PORT))
   client.send(message_1st.encode())
   #print(message_1st.encode())
   connection = True
except:
    print('Unconnected ,please retry')

try:
    serverMessage = str(client.recv(4096), encoding='utf-8')
    print('Server:', serverMessage)
except:
    print('Server on respond')
...
while joyGetPosEx(0, p_info) == 0:
    ...
    x = round((info.dwXpos - 32767) / 32768.0,3)
    rp = round((info.dwZpos - 32767) / 32768.0,3)
    lp = round((info.dwRpos - 32767) / 32768.0,3)

    data.append(int(x*1000))
    data.append(int((lp*1000)-(rp*1000))/2)
